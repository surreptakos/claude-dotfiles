...
- **No worker sessions.** Each master dispatches everything in-session, from inside its repo's
  ...
  `printf %x%x $(date +%s) $$`, because the workflow runtime forbids `Date.now()` in scripts; a
  launch missing any of the three is refused with a contract-mismatch error) is only started AFTER the repo's `/triage` and `/to-tickets`
