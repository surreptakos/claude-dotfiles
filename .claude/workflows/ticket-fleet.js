...
2. Run \`gh api "repos/{owner}/{repo}/pulls?state=open&per_page=100"\`. Never \`gh pr list\`, \`gh pr view\`, \`gh issue list\` or \`gh issue view\` — they are GraphQL-backed and return HTTP 403 in this environment (issue 130).
3. Filter the returned array to entries whose \`head.ref\` starts with \`agent/issue-${t.number}-\`. If any match exists, return {found:true, prUrl:<first match's html_url>, branch:<first match's head.ref>}. If none, return {found:false}.
