...
def _template_copy(path, job):
    parent = os.path.basename(os.path.dirname(path)).lower()
    in_skipped = (os.path.normcase(os.path.dirname(path)) != os.path.normcase(job)
                  and (parent in _SKIP_DIRS or parent.startswith(('_', '.'))))
    return bool(_WU_TEMPLATE_COPY.search(os.path.basename(path).lower())) and not in_skipped
...
def find_workup(job):
    ...
    hits = [h for h in sorted(set(hits), key=os.path.getmtime, reverse=True)
            ...
            and ('template' not in os.path.basename(h).lower() or _template_copy(h, job))
