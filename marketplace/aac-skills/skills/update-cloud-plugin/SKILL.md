...
One packager route, from a claude-dotfiles checkout. Issue 214 retired the live-tree route with
sync push: `aac-skills/` in the repo is the only skill source now, on the desktop and in a
container alike.
...
hash. It defaults to the owner's, which is what CI checks with, so leave it off unless the owner's
home has moved — claude-dotfiles `CLAUDE.md` spells the flag out next to CI's. CI
(`skill-stamps.yml`) checks exactly this: a rebuild from `aac-skills/` must reproduce the committed
payload.
...
so the branch carries the whole publish. **The merge is the release** — every machine and every
cloud session with the repo declared (channels 1 and 2) picks it up on the next
...
report drift, naming the skill — that is the loop closing, not a bug.
