...
- **Step 6 (board sweep) and step 11 (the repo-wide ref sweep)** are not a container's problem any
  more: each is a GitHub Actions job (issues 216 and 474), so read the job's verdict and file no
  `ready-for-local-agent` ticket for either. A session still deletes its own branch, with git.
