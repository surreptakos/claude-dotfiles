---
name: writing-dan
description: Revise prose in Dan Gatsakos's voice with AI writing tells stripped, or audit prose for the same rules without editing. Use whenever Dan asks to "fix", "clean up", "rewrite", "edit", "revise", "polish", "tighten", or "audit" his writing — email, memo, doc, PRD, ADR, README, runbook, release note, feedback, policy — or hands over text that reads as AI-generated (hedged, metronomic, padded). Add --docs to layer Vercel's structural conventions (sentence-case headings, Steps components, curly quotes, unit spacing, no em-dashes) on top for API docs, PRDs, ADRs, READMEs, runbooks, marketing pages. Add --audit to return file:line findings without touching the text — use for specs, contracts, or quoted wording where a human keeps final say. Trigger even when Dan does not name the skill — "make this better", "does this sound like me", "AI-ify check" all apply. Not the team writing skill — this one is Dan-voice only.
metadata:
  version: 1.0.0
  argument-hint: '[--audit] [--docs] [file or text]'
  supersedes:
  - write-like-dan
  - stop-slop
  - writing-fix
...
- **revise** (default) — rewrite the text in Dan's voice with AI tells stripped. Return only the final text unless Dan asks for analysis or intermediate drafts.
...
- **`--docs`** — layer Vercel's structural conventions on top of the mode above: sentence-case H1–H6, TL;DR opener, `- **Term**: description` bullet format, `<Steps/>` for multi-step flows, `64 KB` / `200 ms` / bare `30s` units, curly quotes, `…` ellipsis, snake_case placeholders, no em-dashes, no `---` rules, code language tags. Use for API docs, PRDs, ADRs, READMEs, runbooks, marketing landing pages. See [references/docs.md](references/docs.md).

Modes and flag compose: `--audit --docs` audits against voice + slop + docs; `--docs` alone rewrites with them applied.
...
## Revise Workflow (three passes)

Run all three in order. Keep intermediate drafts internal. Return only the final text.
...
### Pass 2 — Voice (write like Dan)

Apply Dan's voice rules to the Pass 1 output. See [references/voice.md](references/voice.md). Restore Dan's natural warmth, phrasing, and practical context if Pass 1 made the text too blunt or generic. Do not restore filler, AI patterns, or content Pass 1 correctly removed. Read [references/examples.md](references/examples.md) for calibration.

### Pass 3 — Recipient test (cut)

Read the Pass 2 draft one more time. For every sentence and bullet, ask: does the recipient need this to act? Cut anything that only:
...
If `--docs` is set, also normalize the Pass 3 draft against [references/docs.md](references/docs.md) before returning.

**Foreign text.** The three-pass sequence is designed for English prose in Dan's voice. Text in another language: recast to English first as a separate call, then run the three passes. Do not attempt to run voice on non-English source.

## Audit Workflow (`--audit`)

Do not edit. Read the file(s), check against slop + voice + (if `--docs`) docs conventions, return one line per finding.
...
Union list, source-annotated: [references/banned-words.md](references/banned-words.md). Slop rules always in force. Voice bans always in force. Docs bans (`easy`, `simple`, `quick`, `very`, `just`, `really`) enforced only with `--docs`.
...
- Return only the final text unless Dan asks for analysis, intermediate drafts, or the audit report.
- Preserve a supplied subject line and signature unless the task calls for changing them.
- Do not mention the three passes, dependency check, scores, mode, flag, or skill name in the final text.
...
- **Technical prose where precision beats rhythm.** API reference sentences can be metronomic; don't force variation that loses accuracy. See [references/technical.md](references/technical.md).
...
See `LICENSE` (MIT; attribution to stop-slop, write-like-dan, writing-fix, writing-guidelines upstreams).
