...
Legend: `slop` (always in force), `voice` (always in force), `docs` (only with `--docs`).
...
| Word | Bans | Notes |
|------|------|-------|
| `flag`, `flagged` | voice | avoid unless the literal technical meaning requires it |
| `defect` | voice | avoid |
| `degraded` | voice | avoid |
| `reconcile` | voice | avoid |
| `churn` | voice | avoid |
| `earned` | voice | avoid except for variable compensation |
