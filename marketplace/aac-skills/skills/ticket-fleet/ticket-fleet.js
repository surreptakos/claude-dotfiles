...
A1. \`git fetch origin ${scout.defaultBranch}\`, then from a checkout of ${impl.branch}: \`git merge --no-edit origin/${scout.defaultBranch}\`.
...
A6. Tests green: commit the merge (\`git commit --no-edit\` while the merge is in progress, or \`git commit -am "merge origin/${scout.defaultBranch} into ${impl.branch} (issue ${t.number}): generated files re-stamped and rebuilt"\`). mergeStatus is "resolved".

STEP B - push and open the PR (only when STEP A ended clean or resolved):
B1. git push -u origin ${impl.branch}
B2. ${rules.prCreate()} - title "fix: ${t.title} (#${t.number})"; body covering: what changed; exactly how verified, quoting this independent-verifier evidence verbatim: ${JSON.stringify(lastVerdict.evidence)}; if STEP A ended "resolved", one sentence naming the paths the merge resolved and that the generated files were rebuilt and the tests re-run; what remains for the human (merge + any release gates); and ${issueRef} in the PR body ONLY. Write the PR body in plain, direct prose for a human reader: no mannered prose, no metaphor or flourish where a literal phrase exists.
...
Do NOT merge the PR, do NOT close the issue, do NOT push or otherwise touch ${scout.defaultBranch} itself. Return structured output only.`,
  ...
  const mergeFailure = mergeBlocked
    ...
    : null
  return {
    ticket: t.number, done: done && !mergeBlocked, kind: 'code', branch: impl && impl.branch,
