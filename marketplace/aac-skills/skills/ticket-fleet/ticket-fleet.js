...
// ---- Supervisor hard-rule list (issue 628) ----
// The supervisor (orchestrator) enforces these rules in this exact order when selecting which
// tickets enter the wave. The fleet script implements all of them; this list documents what the
// supervisor must know about to size a wave correctly.
//
// Hard rules that BLOCK or SKIP a ticket (checked in this order):
//   1. Open blockers: a ticket whose blockedBy list names an open issue is SKIPPED (dropped before
//      wave selection, logged in run result as skippedBlocked). The scout reads each blocker's
//      state through the tracker, so re-running a wave whose blockers landed handles the retest.
//      Issue 403.
//   2. Maybe Someday milestone: a label-driven wave SKIPS any ticket in the "Maybe Someday"
//      milestone (dropped before wave selection, logged as skippedParked). Explicit tickets
//      (args.tickets) bypass this rule and run regardless of milestone. Issue 786.
//   3. Open fleet PR: any ticket that already carries an open PR whose head starts with
//      `agent/issue-<N>-` is SKIPPED (dropped before wave selection, logged as skippedOpenPR).
//      The scout reads the repo's open PRs once and applies this check so that maxTickets slots
//      fill with tickets that will actually run. Issue 430.
//   4. Handoff pending: a ticket whose latest comment is a fleet handoff (a "Remaining for X"
//      section with the _Generated by Claude Code_ footer) with no owner comment after is PARKED
//      (dropped before wave selection, logged as skippedAwaitingOwner). Re-running without an
//      owner reply would repeat the handoff comment. Issue 266.
//
// Hard rules that ORDER or LIMIT remaining tickets:
//   5. Wave cap: after all skips and parks, the remaining eligible tickets (those with no open
//      blockers, no Maybe Someday milestone, no open fleet PR, and no pending handoff) are sorted
//      by issue number ascending, then wave selection takes the first maxTickets of them. The rest
//      are logged as overCap - runnable but postponed. Every eligible ticket eventually runs once
//      blockers land and the previous wave completes.
//   6. Discovery-triage sequential: eligible tickets marked discoveryTriage:true (chores that file
//      findings as new tracker issues) are queued into one lane and run one after another, not in
//      parallel. A prober or implementer brief for such chores includes a dedupe rail: search the
//      tracker for a matching entry before filing, and comment on a match instead of creating a
//      second ticket. Two such chores running side by side cannot see each other's new tickets, so
//      the serial execution ensures the second reads a tracker the first has already updated.
//      Issue 319.
//
// All six rules combine to ensure that only runnable, unblocked, not-already-running tickets enter
// the wave, in number-order, with discovery chores running one at a time. A supervisor sizing the
// wave to "get ALL ready for agent tickets in one wave" must account for all six rules in order:
// blockers must land first, parked tickets must have owner replies, explicit-ticket lists bypass
// parking and milestone rules, and discovery chores reserve one slot each.
