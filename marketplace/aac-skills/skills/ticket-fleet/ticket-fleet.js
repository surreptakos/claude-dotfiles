...
A7. MARKER SCAN - the last gate before anything is pushed (issue 514). Run \`git grep -l -E '^(<<<<<<< |>>>>>>> |=======$)' HEAD\`. Exit 1 with no output is the pass. Any path it prints is an unresolved conflict that a \`git add -A\` swallowed into the merge commit: in run 6aab1eac the deliverer pushed exactly such a commit for #472, markers and all, because nothing re-read the tree after the resolve. Do NOT push it. Re-resolve each listed path by its class from A3 and \`git add -- <path>\`, then \`git commit --amend --no-edit\` - HEAD is not on origin yet, so amending it rewrites nothing anyone has - and run the scan again. If any listed path is class (c), or the scan still prints a path after one amend, return {pushed:false, prUrl:"", mergeStatus:"blocked", conflictPaths:[every path the scan printed], blockedReason:"conflict markers left in the merge result: <paths>"}.
A8. A BAD COMMIT THAT IS ALREADY ON ORIGIN IS REPAIRED FORWARD, NEVER FORCE-PUSHED (issue 514). \`git grep -l -E '^(<<<<<<< |>>>>>>> )' origin/${branch}\` says whether the published head carries markers. If it does, that history is public: \`git push --force\`, \`--force-with-lease\`, \`git reset\` plus a force push, and asking the session to force-push for you are all forbidden, here and everywhere else in this task. Repair it with a follow-up commit whose TREE is the corrected merge:
   $ CORRECTED=\$(git rev-parse HEAD)      # the merge you resolved above, local only
   $ git reset --hard origin/${branch}     # back onto the published bad head
   $ git read-tree -u --reset \$CORRECTED  # index and working tree := the corrected tree
   $ git commit -m "repair merge <bad-sha> (issue ${t.number}): conflict markers resolved"
   The result is a fast-forward over the published head, so STEP B's plain \`git push\` takes it. Re-run A7 and \`${testCommand}\` on that commit before going on; a non-zero test exit blocks exactly as A5 does.

STEP B - push and open the PR (only when STEP A ended clean or resolved):
B1. Push the branch: \`git push -u origin ${branch}\`. The Implement step pushed it already, so this is normally up to date or a fast-forward - but it MUST succeed here, and "the branch does not exist" is never the answer. A non-zero exit stops delivery loudly: run \`git ls-remote --heads origin ${branch}\` and \`git branch -a --list '*${branch}*'\`, then return {pushed:false, prUrl:"", mergeStatus:"blocked", conflictPaths:[], blockedReason:"push failed: <the git output of all three commands, VERBATIM>"}. Never report a delivery that pushed nothing, and never conclude that the branch, or the issue, does not exist: say what git said. A rejected push is never answered with force: it is either the fast-forward repair of A8 or a blocked return.
