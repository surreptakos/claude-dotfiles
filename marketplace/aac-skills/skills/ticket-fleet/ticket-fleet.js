...
function trackerRules(mode) {
  ...
  if (mode === 'mcp') return {
    repoNote: REPO,
    scoutList: (label) => `${REPO} mcp__github__list_issues with label "${label}", state open, perPage 100, paging until the tool reports no next page - take EVERY matching ticket, the wave has no cap. mcp__github__list_issues does not return pull requests, but if any entry it hands back carries a pull_request field anyway, drop it: a PR is not a ticket. (then mcp__github__issue_read with method get_comments per ticket - comments carry criteria the body lacks).`,
  ...
  return {
    ...
    scoutList: (label) => `\`gh api "repos/{owner}/{repo}/issues?labels=${label}&state=open&per_page=100&page=P"\` for P = 1, 2, ... until a page returns fewer than 100 entries - take EVERY matching ticket, the wave has no cap. This endpoint's entries include pull requests as well as issues; an entry carrying a \`pull_request\` key is a PR, not a ticket - drop it before you read further. Then per surviving ticket N \`gh api repos/{owner}/{repo}/issues/N\` and \`gh api repos/{owner}/{repo}/issues/N/comments\` - comments carry criteria the body lacks.`,
