...
test('the claude-md-lint skill carries a byte-identical copy of the linter', () => {
  ...
  const mirror = path.join(__dirname, '..', 'claude', 'skills', 'claude-md-lint', 'claude-md-lint.js');
  if (fs.existsSync(mirror)) {
    assert.equal(fs.readFileSync(mirror, 'utf8'), here, `${mirror} differs from tools/claude-md-lint.js — the mirror is regenerated by sync.ps1 -Mode push from the live skill tree; rerun that after the live and tools/ copies agree`);
  }
});
