...
test('CLI exit code: warn-only findings print but do not gate; gating findings still do', () => {
  ...
  // finding suppressed in-file does not reach the exit code at all.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'claude-md-lint-warn-'));
  try {
    const root = path.join(dir, 'CLAUDE.md');
    const over = Array.from({ length: 30 }, (_, i) => `- rule number ${i} about thing ${i}`).join('\n');
    fs.writeFileSync(root, `${over}\nAlways write clean code.\n`);
    const bad = spawnSync(process.execPath, [CLI, root, '--max-lines', '10'], { encoding: 'utf8' });
    assert.equal(bad.status, 1);
    assert.match(bad.stdout, /self-evident/);
    assert.match(bad.stdout, /^.*\tsize\t/m); // size prints alongside it...
    assert.match(bad.stdout, /1 gating/);       // ...without counting towards the exit code

    fs.writeFileSync(root, `${over}\n<!-- claude-md-lint-ignore -->\nAlways write clean code.\n`);
    const suppressed = spawnSync(process.execPath, [CLI, root, '--max-lines', '10'], { encoding: 'utf8' });
    assert.equal(suppressed.status, 0, suppressed.stdout + suppressed.stderr);
    assert.doesNotMatch(suppressed.stdout, /self-evident/);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('CLI: exit 0 clean, 1 findings, 2 usage; repo context and --against', () => {
  ...
  try {
    ...
    assert.equal(spawnSync(process.execPath, [CLI, good, '--warn-only', 'not-a-rule'], { encoding: 'utf8' }).status, 2);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
