...
.DESCRIPTION
    ...
    zoho-source-of-truth) - deliberately delivery-only, because ~/.claude.json is
    NOT in the sync manifest (it holds oauthAccount and other machine-only state).
...
function Set-TrustInvariant {
    ...
    if (-not (Test-Path $StatePath)) {
        Write-Host "  MISSING: $StatePath (launch claude once to create it)"
        return $false
    }
