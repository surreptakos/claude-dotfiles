...
// a stubbed `gh` on PATH. `files` are written into the clone beside the chosen workflows.
function runEndWithJobs(workflows, files) {
...
// A claude-dotfiles checkout (sync.ps1 + lib/manifest.ps1, which also puts the end gate's and pull
// nudge's reads of the default branch in the same process): every job is expected there, so a
// missing workflow is still `!!` (issue 891).
test('tracker jobs: --end prints one line per job — ok, STOP, and a single !! for a missing file',
  { skip: process.platform === 'win32' && 'the stub gh is a POSIX shell script' }, () => {
    const plain = runEndWithJobs(['closure-guard', 'board-sweep', 'issue-metadata-audit', 'tracker-audit'],
      { 'lib/manifest.ps1': '# stub\n', 'sync.ps1': '# stub\n' });
...
// Issue 891: a harnessed repo gets only the tracker-audit template, so the four dotfiles-only jobs
// it never had print nothing, while the job it does run is still read.
test('tracker jobs: --end on a harnessed repo prints no !! for the four jobs the harness never ships',
  { skip: process.platform === 'win32' && 'the stub gh is a POSIX shell script' }, () => {
    const plain = runEndWithJobs(['tracker-audit'], { 'docs/agents/issue-tracker.md': '# tracker\n' });
    assert.doesNotMatch(plain, /ReferenceError|TypeError/);
    for (const label of ['closure guard', 'board sweep', 'issue metadata audit', 'stale-ref sweep']) {
      assert.doesNotMatch(plain, new RegExp(`^\\s*!!\\s+${label}:`, 'm'));
    }
    assert.doesNotMatch(plain, /nothing runs this job here/);
    assert.match(plain, /^\s*ok\s+tracker audit: success — https:\/\/github\.com\/o\/r\/actions\/runs\/5$/m);
  });
