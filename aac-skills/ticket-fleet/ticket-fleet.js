...
Do NOT close the issue, do NOT edit the repository, do NOT open a PR, do NOT post more than one comment, and never state that a step outside this container was performed. Return structured output only.`,
  ...
  return {
    ticket: t.number, done: !!handoff, kind: 'human', branch: null, deliveryFailure,
    verdict: handoff
...
Do NOT merge, do NOT close the issue, do NOT touch ${scout.defaultBranch}. Do NOT edit the issue body at all and do NOT tick any acceptance box, ticked or otherwise (aac-routines issue 264): a ticked box claims the work shipped, the work ships at merge, and where this repo has a tick-acceptance-boxes merge workflow that workflow ticks them then. Return structured output only.`,
    ...
    if (!deliveryFailure && !(delivery && delivery.prUrl)) {
    ...
    log(deliveryFailure || `deliver:#${t.number}: ${delivery.prUrl}`)
  ...
  return { ticket: t.number, done, kind: 'code', branch: impl && impl.branch, verdict: lastVerdict, prUrl: delivery && delivery.prUrl, commentUrl: null, deliveryFailure, discoveries: (impl && impl.discoveries) || [] }
}
// [FLEET-CODE-LANE-END]
