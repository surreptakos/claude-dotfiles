...
`%APPDATA%\Claude\claude-code-sessions\<accountUuid>\<orgUuid>\scheduled-tasks.json` (fields:
...
reads each registry at org init and does not rewrite it on Reset, so file edits take effect on
the next switch or restart. aac-routines `scripts/run_stamp.py` is the cross-org guard
