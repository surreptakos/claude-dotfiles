...
linter over one, then walks the trim pass `/doctor` describes for checked-in project files,
extending it to global and uncommitted ones.
...
about a capability trips `code-derivable`. Leave those standing. A suppression comment costs
context of its own, so spend one only where a line must stay and the finding will keep firing
(`<!-- claude-md-lint-ignore -->` on the line above, or `<!-- claude-md-lint-disable: rule,rule -->`
once per file).
...
2. Edit the live file, re-run the linter, and re-run any claims tripwire the repo keeps on that
   file.
...
4. If the file is mirrored into a dotfiles repo, the sync push is what regenerates the mirror —
   the live file is the one copy that gets edited.
