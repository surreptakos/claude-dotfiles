...
description: "Full holistic pass over every memory file — merge duplicates, fix stale facts, prune the index. Every run walks the whole set."
metadata:
...
Every invocation walks the complete memory set. Overlaps, stale facts and durable/dated confusion accumulate in the files that look quiet, so the run opens every file in the directory — the consolidation is done when the number read equals the number present, and the report states both. A scope drawn from the last consolidation, the last N days, or a diff against a prior report is a different job.

## Phase 1 — Take stock (whole directory, every time)

- List the memory directory. Read every file in it.
...
State: total files in the directory, files read this run (equal to the total), files merged, files retired, `MEMORY.md` resulting line count and byte count.
