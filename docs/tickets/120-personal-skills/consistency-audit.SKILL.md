...
0. **Every run is a full holistic sweep.** Scope is the complete prose inventory, every time.
   Drift accumulates hardest in the surfaces nobody has touched, so the sweep is sized by what the
   repo holds today rather than by a git-log window, a recent tracker mention or a prior audit
   report — a prior report is a comparison point at the end. Phase 1's inventory count matching the
   repo's actual prose surface count is the proof; a smaller count restarts the audit.
...
4. **Dated history is not stale.** A handoff or report section describing what was true on its
   date stays as written, and history is corrected by addition: append a dated update line, or head
   the whole doc "historical record — kept for X" when it no longer describes the present. Anything
   that records a measurement is kept.
...
7. **Owner decisions become tickets.** Anything the audit surfaces that needs a human ruling (a
   threshold, a judgment call, a manual UI step) goes through the project's flow (`/triage` or
   `/to-tickets`) and comes back with a ticket number the report can cite.

## Phase 1 — inventory every prose surface

Build the list before reading anything, so nothing is skipped silently. A surface that existed
last sweep and still exists gets re-verified this sweep, even if untouched:
...
Read every surface from Phase 1 for claims a tool can verify. The recurring kinds:
...
- Code claims: `Grep`/`Read` the actual code, and re-derive file:line from what it says.
...
- Full-sweep confirmation: the total inventory count for this run, and the count verified — equal
  numbers are what close the sweep.
