...
description: Rebuild and republish the aac-skills plugin (the single package built from ~/.claude/skills plus the repo aac-skills/ tree). Use when the session-end cloud-skills sweep reports drift or no recorded upload, when the user says the cloud sessions are missing a skill, or after adding or editing a skill that should reach claude.ai/code and Cowork.
metadata:
...
the private marketplace at `surreptakos/claude-dotfiles`. `dan-skills` is retired.
...
3. **Account Skills pages** — individual skills uploaded at `claude.ai/settings/customize` > Skills
   ...
   packager. Org-scoped and personal uploads look identical in the manifest; that scope lives only
   in claude.ai.

**A cloud claude.ai/code container reads channel 2 and nothing else — the repo's
`.claude/settings.json`.** The claude.ai account-level plugin sync returns zero plugins for the
account (`plugins_sync_no_changes count:0` in the session diag log), and the cloud environment setup
script runs before the session's git credentials exist, so `claude plugin marketplace add` fails
there on a private clone. An account-level upload therefore reaches Cowork, not these containers.
...
rebuilds `marketplace/aac-skills/`), and the marketplace push IS the upload for every skill the
plugin serves, so the stamp needs no command of its own. After `git push`, confirm:
...
The zip fallback (step 4) is separate: it reaches the claude.ai Skills pages (channel 3) alone,
which the sweep does not track, so it leaves the stamp where it is.
