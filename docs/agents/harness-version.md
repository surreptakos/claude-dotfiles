...
Installed/upgraded: 2026-09-16 by the `project-harness` skill (v25: `tracker-audit.js`'s `## Blocked by` section ends at its claim, not at end-of-body, issue 364 — backfilled with the missing v23 row by issue 434; v24: the pre-commit gate unsets `GIT_PREFIX` alongside the rest of git’s hook environment, issue 406; v23: no test-suite spawn inherits `NODE_TEST_CONTEXT`, issue 395; v22 narrowed two tracker-audit advisories for meta-tickets, issue 374; v21 added `deleted-subject?`, issue 361; v20 made the harness `tracker-audit.js` a generated copy of this repo's own, issue 336).
