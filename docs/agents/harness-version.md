...
Installed/upgraded: 2026-09-22 by the `project-harness` skill (v33: the harness ships the tracker-audit job as well as the auditor - `templates/tracker-audit.yml`, `templates/tracker-audit-job.js` and its test - so a harnessed repo stops reading `no .github/workflows/tracker-audit.yml` at both ends of every session, issue 675; v32: `add-cloud-plugin.js` lists `Workflow`, `Agent`, `Task` and `mcp__Claude_Code_Remote__*` in `permissions.allow`, so an unattended Routine master dispatching a fleet is never sent to the classifier for an approval nobody is there to give, issue 651; v31: the bootstrap hook seats a copy of itself at `~/.claude/hooks/aac-bootstrap.sh` in user settings, so a session whose project dir is not a harnessed repo - a Routine-fired session with two repository sources, or none - still bootstraps instead of running on whatever the container image held, and `session-check` warns when a healthy marker predates this container, issue 643; v30: the cloud bootstrap hook is wired as `bash "<path>"` and staged 100755 so a Windows commit cannot silence it, and the governance hooks it merges are seated for settings.json instead of carrying the `${CLAUDE_PLUGIN_ROOT}` Claude Code refuses there, issue 614; v29: the auto-mode rule covers
