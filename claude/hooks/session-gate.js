...
const START_RULES = [
  ...
  '- "!! clasp credential needs re-authorizing": deploys blocked. Run the project\'s',
  '  `tools/clasp-auth.js`; never substitute a bare `clasp login` (narrower scopes, shared file).',
...
const END_RULES = [
  ...
  '- "!! tracker audit: N drift finding(s)": check which are from this session.',
  '',
  ...
  'batch those through /to-tickets (breakdown -> approval -> publish), never ad-hoc gh issue',
  'create. A summary naming leftovers without ticket numbers is the failure this prevents.',
  '',
