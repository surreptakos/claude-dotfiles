...
## Revise Workflow (two passes)

Run both in order. Keep intermediate drafts internal. Return only the final text.
...
If `--docs` is set, also normalize the Pass 2 draft against [references/docs.md](references/docs.md) before returning.
...
- Return only the final text unless the user asks for analysis, intermediate drafts, or the audit report.
- Preserve a supplied subject line and signature unless the task calls for changing them.
- Do not mention the two passes, dependency check, scores, mode, flag, or skill name in the final text.
