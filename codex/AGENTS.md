...
- **(4, the one to use since 2026-09-09) the script deploys itself and runs on request:** every AAC Apps Script repo carries `gas.json` + a vendored `SelfDeploy.js` (claude-dotfiles `gas/`). A merge to the default branch is the release; `node <claude-dotfiles>/gas/cli/gas.js run owner/repo <fn> '[args]' --wait 10` runs a function on HEAD and prints the answer (a commit status + comment). `clasp push` and `clasp run-function` are retired.
