...
- **Enumerate all account Apps Script projects** via Drive `files.list mimeType='application/vnd.google-apps.script'` + Apps Script `projects.getContent`. Bound scripts do not appear in that Drive list — read the container spreadsheet's `.clasp.json` or the sheet menu.
