...
const CONFIG = {
  title: 'REPO_TITLE',              // human name for the dashboard heading
  testCommand: null,                // e.g. 'node tests/run-all.js' or 'npm test'; null = no test line
  adrDir: 'docs/adr',               // null = no ADR section
  deployWorkflow: null              // e.g. 'deploy.yml'; null = no deploy line
};
// -----------------------------------------------------------------------------
...
const sha = (process.env.GITHUB_SHA || sh('git rev-parse HEAD').trim()).slice(0, 7);
...
const md = [
  ...
  '',
  '_Generated ' + new Date().toISOString() + ' at commit `' + sha + '` by `scripts/build-dashboard.js` (CI: `dashboard.yml`). Do not edit by hand._',
  '',
