 ...
 * unknown). The upload channel is the marketplace push (sync.ps1 -Mode push writes the stamp as a
 * side effect), so no --accounts plumbing here — the tests describe the after-push state only.
 */
...
// --------------------------------------------------------------- fix text
// The single publish route now is `sync.ps1 -Mode push` (which stamps the sweep) followed by
// `git push`. Any old "re-upload at claude.ai" wording would send the operator to the wrong
// surface for a skill the plugin serves.

test('drift fix text points at the sync push, not claude.ai', (t) => {
  const s = scratch();
  addSkill(s.skills, 'alpha');
  run(s.env, ['--stamp', '--quiet']);
  addSkill(s.skills, 'beta');
